...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < goal:
        ...
        while opponent_score < goal:
            if strategy(score, opponent_score) != k:
                return False
            opponent_score += 1
        score += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    1
    """
    max_num_rolls, max_average_score = 0, -1
...
def check_strategies_beat_baseline():
    import random
    random.seed(0)
    assert average_win_rate(tail_strategy) > 0.55
    assert average_win_rate(square_strategy) > 0.6
    assert average_win_rate(final_strategy) > 0.62
...
def final_strategy(score, opponent_score):
    def capped_update(num_rolls, score, opponent_score):
        """Treat all scores over 100 as equal."""
        return min(100, square_update(num_rolls, score, opponent_score))
    average_update = make_averaged(capped_update, 50)
    return max([0, 1, 2, 3, 6], key=lambda k: average_update(k, score, opponent_score))
    # return optimal_strategy(score, opponent_score)
...
def run(*args):
    ...
    parser.add_argument('--check_strategies', '-c', action='store_true',
                        help='Checks that the staff implementations of the strategies beat the baseline')
    ...
    if args.run_experiments:
        run_experiments()
    elif args.check_strategies:
        check_strategies_beat_baseline()
